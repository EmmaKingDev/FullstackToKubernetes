...
class DbService {
    ...
    async insertNewName(name) {
        try {
            ...
            const insertId = await new Promise((resolve, reject) => {
                const query = ("INSERT INTO names (name, date_added) VALUES ($1,$2)", [name, dateAdded]);
    ...
    async deleteRowById(id) {
        try {
            id = parseInt(id, 10); 
            const response = await new Promise((resolve, reject) => {
                const query = ("DELETE FROM names WHERE id =  $1", [id]);
    ...
    async searchByName(name) {
        try {
            const response = await new Promise((resolve, reject) => {
                const query = ("SELECT * FROM names WHERE name = $1", [name])
